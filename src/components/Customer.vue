    ...
    <div class="pa-10">
      <v-overlay :dark="false" :absolute="true" :opacity="0.50" :value="isOverlay" :z-index="5">
        <v-card>
          ...
          <v-card-title primary-title>Create a new customers</v-card-title>
          <v-card>
      ...
      <div class="text-right mb-12">
        ...
        <v-btn class="mt-12" color="primary" @click="isOverlay = !isOverlay">
          <v-icon left>add</v-icon>Add new Customer
        </v-btn>
      </div>
      <v-card>
        <v-card-title>
...
export default {
  ...
  methods: {
    ...
    viewCustomer(item) {
      console.log(item.id);
      this.$router.push({ name: 'view-data-customer', params: { type: 'customers',id:item.id } })
    },
    getCustomerData() {
        this.customers=[]
      db.firestore()
